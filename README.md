...
# Struggles
Abi - We ran into a few obstacles with this project. We spent the first week we were given just going though the equations the algorithm was going to need and making sure that we understood each of the components of the equations. Once we understood these, it was much easier to get through some coding. Towards the end of writing the main Baum - Welch function, we discovered how difficult it was going to be to correctly put the math we were doing into log - space, as well as bringing it back out at the end. Even without putting it into log - space, the results we were getting did not exactly make sense, so we suspect that some math we are doing is wrong somewhere. We went through it many times to determine what we were doing wrong, as well as going through the code that Marcus wrote, and everything looks right, but we are still getting confusing outputs. 

Chris - Because we weren't able to transition our calculations to log space, the program crashes after 10+ runs through the algorithm. Some of the probabilites get too small and it casues an error. When we tried to convert our calculations to log space, we couldnt figure out how to integrate the pseudo counts to avoid log zero issues. If we could fix that, I think everything else would work properly.

# Personal Reflections
## Group Leader - Chris
While I will say that I'm proud of how far we got with this, I will also say that it still doesn't seem to work quite right. Part of that seems to be choices we made weeks ago that we're only just now seeing the consequence of. Part of our plan was to get all of the equations working properly, then go back and figure out how to get everything into log space. That turned out to be harder than we anticipated. As we worked through the weeks, we also started to use arrays more and more, which some of our older code wasn't designed for. That was easier to fix, but there were definitely time where it looked like it might be easier to write entirely new function instead of adapting our previous work. Overall, I really enjoy this kind of large scale project, and I feel like with a bit more time, we could iron out the last of our issues.
