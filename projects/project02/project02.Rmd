...
### Note
create a`list` of unique occurrence of each word in the `string`
create an empty `dict`
find location of  each occurrence of a word from the `list` in the original `string`
identify the word after it
add the word as a key to the `dict` if it doesn't exist and set its value to `1`
otherwise, increase its value by `1`


Identify the first and last word of the `string`
add `'*S*'` as a key to the `dict` and the first word of the `string` as a transition occurrence.
add `'*E*'` as a transition occurrence from the last word of the `string`
...
### Notes
Use recursion, see Fibonacci
order is number of recursion, and the core `else` statement is our original model
...
use `current_word` to search our outer key of our dict
calculate the probability of each of the possible following words
use a random number generator to choose the next word
continue until we hit an `*E*`
